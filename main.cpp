...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	for (size_t i = 0; i < adapters.size(); i++) {
		...
		//アダプターの情報を取得する
		adapters[i]->GetDesc3(&adapterDisc);
		//ソフトウェアデバイスを回避
	...
	rtvHeapDesc.Type = D3D12_DESCRIPTOR_HEAP_TYPE_RTV;//レンダーターゲットレビュー
...
#ifdef _DEBUG
	ID3D12InfoQueue* infoQueue; if (SUCCEEDED(device->QueryInterface(IID_PPV_ARGS(&infoQueue)))) {
